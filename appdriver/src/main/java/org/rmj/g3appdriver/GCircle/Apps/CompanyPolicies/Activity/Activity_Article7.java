...
public class Activity_Article7 extends AppCompatActivity {
    ...
    private void filter(String text) {
        ...
        for (Violation violation : fullViolationList) {
            if (violation.getName().toLowerCase().contains(text.toLowerCase())) {
    ...
    public List<Violation> loadViolations(Context context) {
        ...
        try {
            InputStream is = context.getAssets().open("article7.json");
